...
const getGlobalReadinessState_1 = require("../lib/engine/getGlobalReadinessState");
const calculateNutrition_1 = require("../lib/engine/calculateNutrition");
const calculateWeight_1 = require("../lib/engine/calculateWeight");
const getHydrationProtocol_1 = require("../lib/engine/getHydrationProtocol");
const adjustForBiology_1 = require("../lib/engine/adjustForBiology");
const calculateWeightCut_1 = require("../lib/engine/calculateWeightCut");
const calculateSchedule_1 = require("../lib/engine/calculateSchedule");
const calculateSC_1 = require("../lib/engine/calculateSC");
const calculateCamp_1 = require("../lib/engine/calculateCamp");
const calculateConditioning_1 = require("../lib/engine/calculateConditioning");
const calculateRoadWork_1 = require("../lib/engine/calculateRoadWork");
// ─── Helpers ───────────────────────────────────────────────────
...
const bugs = [];
function reportBug(day, date, severity, fn, desc, details) {
...
function main() {
    ...
    let campConfig;
    try {
        campConfig = (0, calculateCamp_1.generateCampPlan)({
            fightDate,
    ...
    console.log('\n📋 STEP 2: Generate Weight Cut Plan');
    const planResult = (0, calculateWeightCut_1.generateCutPlan)({
    ...
    const loadHistory = []; // daily loads for ACWR simulation
    ...
    for (let d = 0; d < totalDays; d++) {
        ...
        try {
            ...
            const weightTrend = (0, calculateWeight_1.calculateWeightTrend)({
            ...
            const penaltyResult = (0, calculateWeight_1.calculateWeightReadinessPenalty)(weightTrend, 'fight-camp');
            weightPenalty = penaltyResult.penaltyPoints;
            readinessState = (0, getGlobalReadinessState_1.getGlobalReadinessState)({
                sleep: sleepQuality,
        ...
        if (readinessState === 'Depleted')
            consecutiveDepletedDays++;
        else
            consecutiveDepletedDays = 0;
        // ── 3c. Camp Phase ────────────────────────────────────────
        let campPhase = null;
        let enginePhase = 'fight-camp';
        try {
            const cp = (0, calculateCamp_1.determineCampPhase)(campConfig, currentDate);
            campPhase = cp;
            if (cp)
                enginePhase = (0, calculateCamp_1.toCampEnginePhase)(cp);
        }
        catch (e) {
            reportBug(d, currentDate, 'CRASH', 'determineCampPhase', e.message);
        }
        // ── 3d. Weight Trend + Correction ─────────────────────────
        let weightTrend, weightCorrection;
        try {
            weightTrend = (0, calculateWeight_1.calculateWeightTrend)({
                weightHistory: weightHistory.slice(-14),
                targetWeightLbs: FIGHTER.targetWeight,
                baseWeightLbs: FIGHTER.startWeight,
                phase: enginePhase,
        ...
        try {
            ...
            weightCorrection = weightTrend ? (0, calculateWeight_1.calculateWeightCorrection)({
                weightTrend,
                phase: enginePhase,
                currentTDEE: 2500, // placeholder, we'll use the real one after
                deadlineDate: weighInDate,
            }) : { correctionDeficitCal: 0 };
            nutritionTargets = (0, calculateNutrition_1.calculateNutritionTargets)({
            ...
            if (d === 0) {
                const tdeeWithoutCorrection = (0, calculateNutrition_1.calculateNutritionTargets)({
                    weightLbs: currentWeight,
                    heightInches: FIGHTER.heightInches,
                    age: FIGHTER.age,
                    biologicalSex: FIGHTER.sex,
                ...
                const realCorrection = (0, calculateWeight_1.calculateWeightCorrection)({
                ...
                if (realCorrection.correctionDeficitCal !== weightCorrection.correctionDeficitCal) {
                    reportBug(d, currentDate, 'LOGIC_HOLE', 'calculateWeightCorrection ↔ calculateNutritionTargets', `Circular dependency: Weight correction needs TDEE, but TDEE calculation also needs the correction deficit. ` +
        ...
        // ── 3f. Hydration ─────────────────────────────────────────
        try {
            const hydration = (0, getHydrationProtocol_1.getHydrationProtocol)({
                phase: enginePhase,
                fightStatus: 'pro',
            ...
            if (enginePhase.startsWith('camp-') && hydration.message.includes('Off-season')) {
                reportBug(d, currentDate, 'LOGIC_HOLE', 'getHydrationProtocol', `Camp phase '${enginePhase}' fell through to default in hydration switch. ` +
        ...
        if (isMonday || d === 0) {
            try {
                weekPlan = (0, calculateSchedule_1.generateWeekPlan)({
                    readinessState,
                    phase: enginePhase,
                    acwr: simulatedACWR,
                    recurringActivities,
                ...
                const otWarnings = (0, calculateSchedule_1.detectOvertrainingRisk)(weekPlan.map(p => ({
                    activity_type: p.activity_type,
                    expected_intensity: p.expected_intensity,
                    estimated_duration_min: p.estimated_duration_min,
                    date: p.date,
                })), simulatedACWR, sleepQuality, true);
        ...
        try {
            if (todayActivities.length > 0) {
                const dayValidation = (0, calculateSchedule_1.validateDayLoad)(todayActivities.map(a => ({
        ...
        try {
            for (const act of todayActivities) {
                if (act.expected_intensity >= 7) {
                    const suggestion = (0, calculateSchedule_1.suggestAlternative)({ activity_type: act.activity_type, expected_intensity: act.expected_intensity, custom_label: null }, readinessState, null);
        ...
        try {
            const weeklyVelocity = weightHistory.length >= 7
                ...
                : -1.0;
            cutProtocol = (0, calculateWeightCut_1.computeDailyCutProtocol)({
                plan: planRow,
                date: currentDate,
                currentWeight,
                weightHistory: weightHistory.slice(-14),
            ...
            const finalDayMacros = (0, calculateNutrition_1.resolveDailyMacros)(nutritionTargets, cutProtocol, todayActivities.map(a => ({
                activity_type: a.activity_type,
                expected_intensity: a.expected_intensity,
                estimated_duration_min: a.estimated_duration_min,
            })));
            ...
            if (cutProtocol.trainingIntensityCap !== null) {
                ...
                if (violators.length > 0) {
                    reportBug(d, currentDate, 'INCONSISTENCY', 'trainingIntensityCap enforcement', `Cut protocol caps training at intensity=${cutProtocol.trainingIntensityCap} but ` +
        ...
        try {
            if (todayActivities.length > 0) {
                const nutritionAdj = (0, calculateSchedule_1.adjustNutritionForDay)(nutritionTargets, todayActivities.map(a => ({
                    activity_type: a.activity_type,
                    expected_intensity: a.expected_intensity,
                    estimated_duration_min: a.estimated_duration_min,
        ...
        try {
            if (cutProtocol) {
                const baseHydration = (0, getHydrationProtocol_1.getHydrationProtocol)({
                    phase: enginePhase,
                    fightStatus: 'pro',
                    currentWeightLbs: currentWeight,
                    targetWeightLbs: FIGHTER.targetWeight,
                });
                const cutHydration = (0, getHydrationProtocol_1.getCutHydrationProtocol)({
                ...
                if (Math.abs(cutProtocol.waterTargetOz - cutHydration.dailyWaterOz) > 10) {
                    if (d % 14 === 0) {
                        reportBug(d, currentDate, 'INCONSISTENCY', 'Duplicate hydration targets', `computeDailyCutProtocol.waterTargetOz = ${cutProtocol.waterTargetOz} oz, ` +
        ...
        try {
            ...
            if (scToday) {
                const focus = (0, calculateSC_1.determineFocus)(dayOfWeek, readinessState, enginePhase);
                const workout = (0, calculateSC_1.generateWorkout)({
                    readinessState,
                    phase: enginePhase,
                    acwr: simulatedACWR,
                    exerciseLibrary: mockExerciseLibrary,
                ...
                if (cutProtocol?.trainingIntensityCap && workout.exercises.length > 0) {
                    ...
                    if (maxRPE > cutProtocol.trainingIntensityCap) {
                        reportBug(d, currentDate, 'INCONSISTENCY', 'generateWorkout intensity cap', `Cut caps intensity at ${cutProtocol.trainingIntensityCap} but workout has exercise at RPE ${maxRPE}.`);
                    }
                }
            }
        }
        ...
        try {
            ...
            if (condToday) {
                (0, calculateConditioning_1.prescribeConditioning)({
                    phase: enginePhase,
                    fitnessLevel: FIGHTER.fitnessLevel,
                    readinessState,
                    acwr: simulatedACWR,
        ...
        try {
            ...
            if (runToday) {
                (0, calculateRoadWork_1.prescribeRoadWork)({
                    phase: enginePhase,
                    fitnessLevel: FIGHTER.fitnessLevel,
                    readinessState,
                    acwr: simulatedACWR,
        ...
        if (d === 0) {
            try {
                (0, adjustForBiology_1.adjustForBiology)({ cycleDay: 1 });
                (0, adjustForBiology_1.adjustForBiology)({ cycleDay: 14 });
                (0, adjustForBiology_1.adjustForBiology)({ cycleDay: 22 });
            }
            catch (e) {
            ...
            try {
                (0, adjustForBiology_1.adjustForBiology)({ cycleDay: 0 });
