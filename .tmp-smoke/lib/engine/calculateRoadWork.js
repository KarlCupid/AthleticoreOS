...
exports.getRoadWorkType = getRoadWorkType;
exports.prescribeRoadWork = prescribeRoadWork;
exports.calculateRunningLoad = calculateRunningLoad;
exports.getWeeklyRoadWorkPlan = getWeeklyRoadWorkPlan;
 ...
 */
const TYPE_DURATION_MULTIPLIERS = {
    easy_run: 1.0,
...
function isHighCNS(type) {
    return CNS_LOAD_BY_TYPE[type] >= 6;
}
function getEstimatedMaxHR(age) {
    return age !== null ? Math.round(220 - age) : null;
}
function buildPaceGuidance(type, maxHR) {
...
function buildProgressionNote(type, weekNum) {
...
function buildMessage(type, fitnessLevel, phase) {
...
function buildIntervals(type, fitnessLevel) {
    if (type === 'intervals') {
        const repsByLevel = { beginner: 4, intermediate: 6, advanced: 8, elite: 10 };
 ...
 */
function getRoadWorkType(phase, readinessState, acwr, sessionIndex = 0) {
 ...
 */
function prescribeRoadWork(input) {
    const { phase, fitnessLevel, readinessState, acwr, age, sessionIndex = 0, activeCutPlan, trainingIntensityCap, trainingIntensityCapOverride, campConfig, } = input;
    // Determine run type
    ...
    const effectiveDate = (0, date_1.todayLocalDate)();
    const effectiveIntensityCap = resolvedIntensityCap !== undefined
        ? resolvedIntensityCap
        : (0, calculateWeightCut_1.getDailyCutIntensityCap)(activeCutPlan, effectiveDate);
    ...
    if (effectiveIntensityCap !== null && effectiveIntensityCap !== undefined) {
        ...
        if (startIdx !== -1) {
            for (let idx = startIdx; idx < intensityLadder.length; idx++) {
                ...
                if (idx === intensityLadder.length - 1)
                    type = 'recovery_jog';
            }
        }
    }
    ...
    const typeDistanceMultipliers = {
        recovery_jog: 0.5,
        easy_run: 1.0,
        long_slow_distance: 1.7,
        tempo: 0.6,
        intervals: null, // no fixed distance for intervals
        hill_sprints: null,
    };
    ...
    const totalDurationMin = Math.max(20, Math.round(baseDurationMin * TYPE_DURATION_MULTIPLIERS[type]));
 ...
 * @ANTI-WIRING: Pure synchronous function.
 */
function calculateRunningLoad(prescription) {
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function getWeeklyRoadWorkPlan(input) {
    if (input.prescriptionsNeeded <= 0)
        return [];
    const { weekStartDate, prescriptionsNeeded, recurringActivities, existingActivities, fitnessLevel, phase, readinessState, acwr, age, campConfig, activeCutPlan, } = input;
    // Build a map of existing day loads this week
    const allTemplateActivities = recurringActivities.filter(t => t.is_active && t.recurrence.frequency === 'weekly');
    const dayData = [];
    for (let offset = 0; offset < 7; offset++) {
        const date = addDays(weekStartDate, offset);
        const dayOfWeek = dateFromISO(date).getDay();
        const templateForDay = allTemplateActivities.filter(t => t.recurrence.days_of_week?.includes(dayOfWeek));
        const existingForDay = existingActivities.filter(a => a.date === date);
        const allForDay = [...templateForDay, ...existingForDay];
        const hasHighCNS = allForDay.some(a => a.activity_type === 'sparring' || a.activity_type === 'sc');
        const totalLoad = existingForDay.reduce((sum, a) => sum + (a.estimated_duration_min * a.expected_intensity), 0);
    ...
    for (let i = 0; i < Math.min(prescriptionsNeeded, candidateDays.length); i++) {
        ...
        const prevIsIntense = result.some(r => r.date === prevDate && isHighCNS(r.prescription.type));
        const nextIsIntense = result.some(r => r.date === nextDate && isHighCNS(r.prescription.type));
        // If surrounded by high intensity, downgrade this session to easy
        const forceEasy = prevIsIntense && nextIsIntense;
        const trainingIntensityCap = (0, calculateWeightCut_1.getDailyCutIntensityCap)(activeCutPlan, day.date);
