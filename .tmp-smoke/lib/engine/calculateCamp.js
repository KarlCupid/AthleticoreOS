...
exports.generateCampPlan = generateCampPlan;
exports.determineCampPhase = determineCampPhase;
exports.getCampTrainingModifiers = getCampTrainingModifiers;
exports.getCampWeekProfile = getCampWeekProfile;
exports.toCampEnginePhase = toCampEnginePhase;
exports.getAutoTaperMultiplier = getAutoTaperMultiplier;
exports.getCampSCModifier = getCampSCModifier;
exports.getSparringDayGuidance = getSparringDayGuidance;
 ...
 */
const PHASE_VOLUME_MULTIPLIERS = {
 ...
 */
function generateCampPlan(input) {
 ...
 */
function determineCampPhase(config, dateStr) {
    if (dateStr < config.campStartDate || dateStr > config.fightDate) {
        return null;
    }
    if (dateStr <= config.basePhaseDates.end)
        return 'base';
    if (dateStr <= config.buildPhaseDates.end)
        return 'build';
    if (dateStr <= config.peakPhaseDates.end)
        return 'peak';
    return 'taper';
}
 ...
 * @ANTI-WIRING: Pure synchronous function.
 */
function getCampWeekProfile(config, weekStartDate, fitnessLevel) {
 ...
 * @ANTI-WIRING: Pure synchronous function.
 */
function toCampEnginePhase(campPhase) {
    const map = {
        base: 'camp-base',
        build: 'camp-build',
        peak: 'camp-peak',
 ...
 * @ANTI-WIRING: Pure synchronous function.
 */
function getAutoTaperMultiplier(sparringDaysPerWeek) {
    if (sparringDaysPerWeek <= 1)
        return 1.0;
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function getCampSCModifier(campPhase, sparringDaysThisWeek) {
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function getSparringDayGuidance(campPhase, library) {
    // Find activation exercises: mobility + band work
    const mobilityExercises = library.filter(ex => ex.type === 'mobility' || ex.type === 'active_recovery');
    // Find band/bodyweight exercises for activation
    const activationCandidates = library.filter(ex => (ex.equipment === 'band' || ex.equipment === 'bodyweight')
        && ex.cns_load <= 3
        && ex.type !== 'heavy_lift'
        && ex.type !== 'power');
