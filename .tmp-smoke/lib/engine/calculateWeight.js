...
exports.calculateWeightCorrection = calculateWeightCorrection;
exports.calculateWeightReadinessPenalty = calculateWeightReadinessPenalty;
...
function getWeeklyVelocitySeries(weights) {
    const series = [];
    if (weights.length < 8)
        return series;
...
function confidenceFromHistory(historyLen, velocityStd) {
    if (historyLen >= 21 && velocityStd <= 0.35)
        return 'high';
    if (historyLen >= 14 && velocityStd <= 0.7)
        return 'medium';
    return 'low';
}
// calculateWeightTrend
function calculateWeightTrend(input) {
    ...
    let projectedDateEarliest = null;
    let projectedDateLatest = null;
    let projectedWeeklyVelocityRange = null;
 ...
 */
function calculateWeightCorrection(input) {
 ...
 */
function calculateWeightReadinessPenalty(trend, phase) {
