...
exports.prescribeConditioning = prescribeConditioning;
exports.getWeeklyConditioningPlan = getWeeklyConditioningPlan;
...
const BEGINNER_CIRCUIT_EXERCISES = [
...
function getConditioningType(phase, readinessState, sessionIndex) {
 ...
 */
function prescribeConditioning(input) {
    const { phase, fitnessLevel, readinessState, acwr, sessionIndex = 0, activeCutPlan, trainingIntensityCap, trainingIntensityCapOverride, } = input;
    ...
    const intensityLabel = readinessState === 'Depleted' ? 'light' :
        readinessState === 'Caution' ? 'moderate' :
            'hard';
    switch (type) {
        case 'heavy_bag_rounds': {
            rounds = Math.round(BASE_BAG_ROUNDS[fitnessLevel] *
            ...
            totalDurationMin = Math.round((rounds * (workIntervalSec + restIntervalSec)) / 60);
            exercises = [{
        ...
        case 'jump_rope': {
            const jrByLevel = { beginner: 3, intermediate: 5, advanced: 8, elite: 10 };
    ...
    const message = `${typeLabels[type]}: ${rounds} rounds × ${Math.round(workIntervalSec / 60 * 10) / 10} min. ` +
 ...
 */
function getWeeklyConditioningPlan(input) {
    if (input.prescriptionsNeeded <= 0)
        return [];
    const { weekStartDate, prescriptionsNeeded, recurringActivities, existingActivities, fitnessLevel, phase, readinessState, acwr, campConfig, activeCutPlan, } = input;
    ...
    for (let offset = 0; offset < 7; offset++) {
        ...
        const hasHighCNS = [...templateForDay, ...existingForDay].some(a => a.activity_type === 'sparring' || a.activity_type === 'sc');
        if (hasHighCNS)
            continue;
