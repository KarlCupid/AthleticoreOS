...
exports.assessFitnessFromQuestionnaire = assessFitnessFromQuestionnaire;
exports.deriveFitnessFromHistory = deriveFitnessFromHistory;
exports.getFitnessModifiers = getFitnessModifiers;
// ─── Scoring Thresholds ────────────────────────────────────────
/**
...
function assessFitnessFromQuestionnaire(input) {
    ...
    const volumeScore = clamp(input.weeklySessionCount <= 2 ? input.weeklySessionCount * 8 :
        input.weeklySessionCount <= 4 ? 16 + (input.weeklySessionCount - 2) * 15 :
            input.weeklySessionCount <= 6 ? 46 + (input.weeklySessionCount - 4) * 12 :
                70 + Math.min(input.weeklySessionCount - 6, 6) * 5, 0, 100);
    categories.push({
    ...
    const strengthScore = clamp(input.maxPushUpsIn2Min < 20 ? input.maxPushUpsIn2Min * 1.25 :
        input.maxPushUpsIn2Min < 40 ? 25 + (input.maxPushUpsIn2Min - 20) * 1.25 :
            input.maxPushUpsIn2Min < 70 ? 50 + (input.maxPushUpsIn2Min - 40) * 0.83 :
                75 + Math.min(input.maxPushUpsIn2Min - 70, 30) * 0.83, 0, 100);
    categories.push({
    ...
    if (input.mile5RunTimeSeconds !== null) {
        ...
        cardioScore = clamp(t > 900 ? Math.max(0, 20 - (t - 900) / 30) :
            t > 720 ? 20 + (900 - t) / 6 :
                t > 540 ? 45 + (720 - t) / 6 :
                    70 + Math.min((540 - t) / 6, 30), 0, 100);
    ...
    const compositeScore = Math.round(categories.reduce((sum, cat, i) => sum + cat.score * weights[i], 0));
    // ── Injury penalty ──
    ...
    const confidence = input.mile5RunTimeSeconds !== null ? 'high' :
    ...
    const summary = `Based on your responses, you're classified as ${level.toUpperCase()} — ` +
        `${levelLabels[level]}. ` +
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function deriveFitnessFromHistory(sessions, weeklyTargets, currentLevel) {
    ...
    const spanDays = Math.floor((new Date(lastDate).getTime() - new Date(firstDate).getTime()) / (1000 * 3600 * 24));
    if (spanDays < 14) {
        return currentLevel;
    }
    const weekGroups = [];
    ...
    for (const s of sortedSessions) {
        const daysDiff = Math.floor((new Date(s.date).getTime() - new Date(weekStart).getTime()) / (1000 * 3600 * 24));
        if (daysDiff < 7) {
            currentWeek.push(s);
        }
        else {
    ...
    const weeklySessionTarget = weeklyTargets.sc_sessions +
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function getFitnessModifiers(level, phase) {
