...
exports.generateCutPlan = generateCutPlan;
exports.determineCutPhase = determineCutPhase;
exports.getDailyCutIntensityCap = getDailyCutIntensityCap;
exports.computeCarbCycle = computeCarbCycle;
exports.detectStall = detectStall;
exports.validateCutSafety = validateCutSafety;
exports.computeDailyCutProtocol = computeDailyCutProtocol;
exports.computeRehydrationProtocol = computeRehydrationProtocol;
const getHydrationProtocol_1 = require("./getHydrationProtocol");
...
function today() {
    return (0, date_1.todayLocalDate)();
}
// ─── generateCutPlan ──────────────────────────────────────────
/**
...
function generateCutPlan(input) {
    ...
    if (extremeCutWarning) {
        safetyWarnings.unshift(`⚠️ EXTREME CUT WARNING: This ${totalCutPct.toFixed(1)}% cut (${totalCutLbs.toFixed(1)} lbs) far exceeds the 10% ` +
    ...
    const estimatedDietLoss = chronicRatePerWeek * chronicPhaseWeeks +
        intensifiedRatePerWeek * intensifiedPhaseWeeks;
    if (dietPhaseTargetLbs > estimatedDietLoss * 1.25) {
        safetyWarnings.push(`Your diet phase needs to drop ${dietPhaseTargetLbs.toFixed(1)} lbs, ` +
    ...
    if (totalCutPct > 7) {
        safetyWarnings.push(`This is an aggressive cut (${totalCutPct.toFixed(1)}% body weight). ` +
    ...
    const intensifiedPhaseStart = intensifiedPhaseWeeks > 0
        ? addDays(fightWeekStart, -Math.round(intensifiedPhaseWeeks * 7))
        : fightWeekStart;
    const chronicPhaseDates = chronicPhaseWeeks > 0
        ? { start: todayStr, end: addDays(intensifiedPhaseStart, -1) }
        : null;
    const intensifiedPhaseDates = {
        start: intensifiedPhaseStart,
        end: intensifiedPhaseEnd,
    };
    const fightWeekDates = { start: fightWeekStart, end: weighInDate };
    ...
    const estimatedDailyDeficitChronic = chronicPhaseWeeks > 0
        ? Math.round((chronicRatePerWeek * effectiveCalPerLb) / 7)
        : 0;
    const estimatedDailyDeficitIntensified = Math.round((intensifiedRatePerWeek * effectiveCalPerLb) / 7);
...
// ─── determineCutPhase ────────────────────────────────────────
function determineCutPhase(plan, dateStr) {
    const d = daysBetween(dateStr, plan.weigh_in_date); // positive = future
    if (d < 0)
        return 'rehydration'; // past weigh-in
    if (d === 0)
        return 'weigh_in'; // weigh-in day
    if (d <= 3)
        return 'fight_week_cut'; // days 1-3 out
    if (d <= 7)
        return 'fight_week_load'; // days 4-7 out
    if (plan.intensified_phase_start && dateStr >= plan.intensified_phase_start) {
        return 'intensified';
    }
    return 'chronic';
}
function getDailyCutIntensityCap(plan, dateStr) {
    if (!plan)
        return null;
 ...
 */
function computeCarbCycle(input) {
 ...
 */
function validateCutSafety(input) {
    const { cutPhase, startWeightLbs, currentWeightLbs, weeklyVelocityLbs, prescribedCalories, calorieFloor, consecutiveDepletedDays, acwr, urineColor, bodyTempF, baselineCognitiveScore, latestCognitiveScore, waterCutAllocationLbs, remainingLbsToTarget, daysToWeighIn, } = input;
    const flags = [];
 ...
 */
function computeDailyCutProtocol(input) {
    const { plan, date, currentWeight, weightHistory, baseNutritionTargets, dayActivities, readinessState, acwr, cycleDay, weeklyVelocityLbs, lastRefeedDate, lastDietBreakDate, baselineCognitiveScore, latestCognitiveScore, urineColor, bodyTempF, consecutiveDepletedDays, } = input;
    ...
    const isTrainingDay = dayActivities.some(a => a.activity_type !== 'rest' && a.activity_type !== 'active_recovery');
    ...
    let prescribedCalories;
    let prescribedProtein;
    let prescribedCarbs;
    let prescribedFat;
    let isCarbCycleHigh = false;
    let waterTargetOz;
    let sodiumTargetMg = null;
    let sodiumInstruction;
    let fiberInstruction;
    let trainingIntensityCap = getDailyCutIntensityCap(plan, date);
    let trainingRecommendation;
    let morningProtocol;
    let afternoonProtocol;
    let eveningProtocol;
    ...
    if (cutPhase === 'rehydration') {
        ...
        const dailyCalByDay = { 1: 400, 2: 600, 3: 800 };
 ...
 */
function computeRehydrationProtocol(input) {
