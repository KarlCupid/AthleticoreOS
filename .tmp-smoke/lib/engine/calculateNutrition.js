...
exports.computeMacroAdherence = computeMacroAdherence;
exports.resolveDailyNutritionTargets = resolveDailyNutritionTargets;
exports.resolveDailyMacros = resolveDailyMacros;
const adjustForBiology_1 = require("./adjustForBiology");
const calculateSchedule_1 = require("./calculateSchedule");
/**
 * @ANTI-WIRING:
...
function calculateBMR(weightLbs, heightInches, age, biologicalSex) {
...
function calculateNutritionTargets(input) {
    const { weightLbs, heightInches, age, biologicalSex, activityLevel, phase, nutritionGoal, cycleDay, coachProteinOverride, coachCarbsOverride, coachFatOverride, coachCaloriesOverride, weightCorrectionDeficit, } = input;
    // 1. Calculate TDEE
    ...
    if (cycleDay != null && cycleDay >= 1 && cycleDay <= 28) {
        const biology = (0, adjustForBiology_1.adjustForBiology)({ cycleDay });
 ...
 */
function computeMacroAdherence(actual, prescribed) {
    const safePct = (a, p) => p > 0 ? Math.round((a / p) * 100) : a === 0 ? 100 : 0;
    ...
    const withinRange = (pct, threshold) => Math.abs(pct - 100) <= threshold;
    const allWithin10 = withinRange(caloriesPct, 10) &&
        withinRange(proteinPct, 10) &&
        withinRange(carbsPct, 10) &&
        withinRange(fatPct, 10);
    const allWithin20 = withinRange(caloriesPct, 20) &&
 ...
 */
function resolveDailyNutritionTargets(baseTargets, cutProtocol, dayActivities) {
    ...
    const dayAdj = (0, calculateSchedule_1.adjustNutritionForDay)(baseTargets, dayActivities, null);
    ...
    return {
        ...
        carbs: modifiedCarbs,
        fat: modifiedFat,
        source: dayActivities.length > 0 ? 'daily_activity_adjusted' : 'base',
        message: dayActivities.length > 0 ? dayAdj.message : baseTargets.message,
    };
}
function resolveDailyMacros(baseTargets, cutProtocol, dayActivities) {
