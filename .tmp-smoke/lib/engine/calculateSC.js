...
exports.determineFocus = determineFocus;
exports.scoreExerciseForUser = scoreExerciseForUser;
exports.generateWorkout = generateWorkout;
exports.filterByEquipment = filterByEquipment;
exports.generateWorkoutV2 = generateWorkoutV2;
exports.calculateVolumeLoad = calculateVolumeLoad;
exports.calculateWeeklyVolume = calculateWeeklyVolume;
exports.getWorkoutCompliance = getWorkoutCompliance;
const calculateOverload_1 = require("./calculateOverload");
const calculateWarmup_1 = require("./calculateWarmup");
const adaptiveWorkout_1 = require("./adaptiveWorkout");
// ─── Constants ─────────────────────────────────────────────────
/**
...
function resolveDayOfWeek(trainingDate) {
    if (trainingDate) {
 ...
 */
function determineFocus(dayOfWeek, readinessState, phase, overrideFocus) {
    if (overrideFocus)
 ...
 */
function scoreExerciseForUser(exercise, context) {
...
function generateWorkout(input) {
    const { readinessState, phase, acwr, exerciseLibrary, recentExerciseIds, recentMuscleVolume, focus: overrideFocus, trainingIntensityCap, trainingDate, fitnessLevel, } = input;
    ...
    const workoutType = effectiveFocus === 'recovery' ? 'recovery'
        : effectiveFocus === 'conditioning' ? 'conditioning'
            : effectiveFocus === 'sport_specific' ? 'practice'
                : 'strength';
    ...
    for (const { exercise, score } of scored) {
        ...
        if (score <= 0)
            continue;
        if (exercise.cns_load > cnsBudgetRemaining)
            continue;
 ...
 */
function filterByEquipment(exercises, availableEquipment) {
    if (!availableEquipment)
        return exercises;
    return exercises.filter(e => {
        const eq = e.equipment;
        if (eq === 'bodyweight' || eq === 'other')
            return true;
        if (eq === 'machine')
            return availableEquipment.some(item => MACHINE_ITEMS.includes(item));
...
function estimateWorkoutTime(exercises) {
    const restDefaults = (0, adaptiveWorkout_1.getRestTimerDefaults)();
    let totalMinutes = 0;
 ...
 * @ANTI-WIRING: Pure synchronous function.
 */
function fitToTimeConstraint(exercises, availableMinutes) {
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function generateWorkoutV2(input) {
    const { readinessState, phase, acwr, exerciseLibrary, recentExerciseIds, recentMuscleVolume, focus: overrideFocus, trainingIntensityCap, trainingDate, fitnessLevel, availableMinutes, gymEquipment, exerciseHistory, isDeloadWeek = false, weeklyPlanFocus, sparringDaysThisWeek, isSparringDay = false, progressionModel: inputProgressionModel, } = input;
    ...
    for (const { exercise, score } of scored) {
        ...
        if (history && history.length > 0) {
            const model = inputProgressionModel ?? (0, calculateOverload_1.selectProgressionModel)(fitnessLevel, history.length);
            overloadSuggestion = (0, calculateOverload_1.suggestOverload)({
        ...
        const warmupResult = (0, calculateWarmup_1.generateWarmupSets)({
    ...
    let campPhaseContext = null;
    if (phase.startsWith('camp-')) {
        campPhaseContext = phase.replace('camp-', '');
    }
    return {
        focus: effectiveFocus,
        workoutType,
 ...
 */
function buildSparringDayWorkout(library, gymEquipment, phase) {
    let exercises = library.filter(e => e.type === 'mobility' || e.type === 'active_recovery' ||
        (e.type === 'sport_specific' && e.cns_load <= 3));
    exercises = filterByEquipment(exercises, gymEquipment);
    const activation = exercises.slice(0, 5).map((exercise) => ({
        exercise,
        targetSets: 2,
 ...
 * Pure synchronous function.
 */
function getWorkoutCompliance(plannedSets, actualSets, plannedVolume, actualVolume) {
 ...
 */
function filterByFocus(library, focus) {
    switch (focus) {
        case 'upper_push':
            return library.filter(e => ['chest', 'shoulders', 'arms'].includes(e.muscle_group) ||
                e.type === 'power');
        case 'upper_pull':
            return library.filter(e => ['back', 'arms'].includes(e.muscle_group) ||
                e.type === 'power');
        case 'lower':
            return library.filter(e => ['quads', 'hamstrings', 'glutes', 'calves'].includes(e.muscle_group) ||
                e.type === 'power');
        case 'full_body':
            return library.filter(e => e.type !== 'mobility' && e.type !== 'active_recovery');
        case 'sport_specific':
            return library.filter(e => e.type === 'sport_specific' || e.sport_tags.includes('boxing'));
        case 'conditioning':
            return library.filter(e => e.type === 'conditioning' || e.type === 'power');
        case 'recovery':
            return library.filter(e => e.type === 'mobility' || e.type === 'active_recovery');
        default:
            return library;
