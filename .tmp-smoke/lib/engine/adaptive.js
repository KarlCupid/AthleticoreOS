...
exports.handleTimelineShift = handleTimelineShift;
exports.autoRegulateSC = autoRegulateSC;
// ─── Constants ─────────────────────────────────────────────────
/**
 ...
 */
function handleTimelineShift({ skippedBlock, currentLedger, cutPhase, }) {
    if (skippedBlock.status !== 'Skipped') {
        throw new Error(`handleTimelineShift expects a block with status 'Skipped', got '${skippedBlock.status}'.`);
    }
    // Use amplified coefficient during intensified/fight-week cut phases
    const isAmplifiedCut = cutPhase != null && (cutPhase === 'intensified' ||
    ...
    const carbReduction = skippedBlock.planned_intensity * coefficient;
    // Floor at 0 — never prescribe negative carbs.
    const updatedCarbs = Math.max(0, currentLedger.prescribed_carbs - carbReduction);
    // The actual reduction applied (may be smaller if we hit the floor).
    const appliedReduction = currentLedger.prescribed_carbs - updatedCarbs;
    const cutContext = isAmplifiedCut
        ? ' (amplified for active weight cut)'
        : '';
    const message = `Workout skipped. Your daily carb target has been lowered by ${appliedReduction}g${cutContext} to keep your weight cut on schedule.`;
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function autoRegulateSC({ boxingBlock, next24hBlocks, exerciseLibrary, }) {
    // Guard: only triggers when boxing intensity was genuinely high.
    if (boxingBlock.block_type !== 'Boxing' ||
        ...
        return {
            ...
            message: 'Boxing intensity within normal range. No schedule changes required.',
        };
    }
    ...
    const heavyLiftBlock = next24hBlocks.find((block) => block.status === 'Scheduled' && block.block_type === 'S&C');
    if (!heavyLiftBlock) {
        return {
            swapped: false,
            originalBlockId: null,
            replacementType: null,
            message: 'No heavy lifting scheduled in the next 24 hours. Your recovery window is clear.',
        };
    }
    // Pick the lowest-CNS-load mobility exercise as the replacement.
    const mobilityExercises = exerciseLibrary
        .filter((ex) => ex.type === 'mobility')
    ...
    if (mobilityExercises.length === 0) {
        return {
            ...
            replacementType: null,
            message: 'A heavy lift should be swapped out to protect your CNS, but no mobility exercises are available in the exercise library.',
        };
    }
    const replacement = mobilityExercises[0];
    const message = `Boxing intensity was ${boxingBlock.actual_intensity}/10. ` +
