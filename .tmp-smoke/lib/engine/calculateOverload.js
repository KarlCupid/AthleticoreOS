...
exports.estimateE1RM = estimateE1RM;
exports.suggestOverload = suggestOverload;
exports.detectPR = detectPR;
exports.shouldDeload = shouldDeload;
exports.selectProgressionModel = selectProgressionModel;
// ─── Constants ───────────────────────────────────────────────
/** Upper-body muscle groups receive smaller weight jumps. */
const UPPER_BODY_GROUPS = ['chest', 'back', 'shoulders', 'arms'];
 ...
 */
function isUpperBody(muscleGroup) {
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function suggestOverload(input) {
    const { exerciseId, exerciseName, history, progressionModel, isDeloadWeek, readinessState, targetRPE, targetReps, muscleGroup, } = input;
    // ── Confidence from history depth ──
    const confidence = history.length >= 6 ? 'high' :
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function detectPR(_exerciseId, exerciseName, weight, reps, rpe, existingPRs) {
    const noPR = {
        isNewPR: false,
        prType: null,
        previousBest: null,
    ...
    const repsPRs = existingPRs.filter(pr => pr.prType === 'reps' && pr.weightAtPR !== null && pr.weightAtPR >= weight);
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function shouldDeload(input) {
    const { weeksSinceLastDeload, autoDeloadIntervalWeeks, acwr, recentSessionRPEs, consecutiveCautionDays, } = input;
    const noDeload = {
        shouldDeload: false,
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function selectProgressionModel(fitnessLevel, historyLength) {
