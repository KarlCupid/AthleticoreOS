...
exports.initFatigueState = initFatigueState;
exports.processSetCompletion = processSetCompletion;
exports.findSubstituteExercise = findSubstituteExercise;
exports.getRestTimerDefaults = getRestTimerDefaults;
exports.getRestDuration = getRestDuration;
// ─── Equipment Mapping ──────────────────────────────────────────
/**
 ...
 */
function isEquipmentAvailable(exerciseEquipment, availableEquipment) {
    ...
    if (requiredItem == null)
        return true; // unknown equipment — assume available
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function processSetCompletion(input) {
    const { exerciseId, actualRPE, targetWeight, targetReps, targetRPE, currentFatigueState, remainingExercises, exerciseLibrary, availableEquipment, } = input;
    // ── 1. Compute RPE delta ────────────────────────────────────
    ...
    const consecutiveHighRPESets = rpeDelta >= 1
        ? currentFatigueState.consecutiveHighRPESets + 1
        : 0;
    const rawFatigueScore = (avgRPEDelta * 20) +
    ...
    // RPE delta <= -1: feeling strong — suggest weight increase
        ...
        const increasedWeight = Math.max(Math.round(targetWeight * 1.025), targetWeight + 5);
        adjustments.push({
            exerciseId,
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function findSubstituteExercise(original, library, availableEquipment) {
    const candidates = library
        .filter((ex) => ex.muscle_group === original.muscle_group &&
 ...
 * Pure synchronous function. No database queries. No LLM generation.
 */
function getRestDuration(exerciseType, fatigueLevel) {
