...
export function suggestWeightClass(input: {
  ...
  sport: CutSport;
  maxSafeCutPct?: number;  // override; default = fightStatus-based cap
}): WeightClassSuggestion[] {
  const { currentWeight, fightStatus, sport } = input;
  const hardCutCapPct = input.maxSafeCutPct ?? (fightStatus === 'amateur' ? 8 : 10);
  ...
  for (const wc of classes) {
    ...
    if (cutPct > hardCutCapPct) {
      risk = 'unsafe';
      riskReason = `Requires ${cutPct.toFixed(1)}% cut, exceeding the ${hardCutCapPct}% safe limit.`;
      feasible = false;
    } else if (cutPct > 8) {
      risk = 'high';
      riskReason = `${cutPct.toFixed(1)}% cut is aggressive. Requires disciplined diet + water cut.`;
      feasible = true;
    } else if (cutPct > 4) {
      risk = 'moderate';
      riskReason = `${cutPct.toFixed(1)}% cut is manageable with consistent diet.`;
      feasible = true;
    } else if (cutPct >= 0) {
      risk = 'low';
      riskReason = cutPct < 1 ? 'Essentially your natural weight class.' : `${cutPct.toFixed(1)}% cut is minimal.`;
      feasible = true;
    } else {
      // Athlete is under the limit — no cut required (moving up or already in class)
      risk = 'low';
      riskReason = `You are ${Math.abs(cutRequired).toFixed(1)} lbs under the limit — no cut required.`;
      feasible = true;
    }
