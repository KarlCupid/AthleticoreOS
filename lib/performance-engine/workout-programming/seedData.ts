...
export const movementPatterns: WorkoutTaxonomyItem[] = [
  ...
  { id: 'trunk_flexion', label: 'Trunk Flexion', summary: 'Controlled spinal flexion or anterior trunk work.' },
...
const exerciseRelationships: Record<string, Pick<Exercise, 'regressionExerciseIds' | 'progressionExerciseIds' | 'substitutionExerciseIds'>> = {
  barbell_back_squat: { regressionExerciseIds: ['goblet_squat', 'box_squat'], progressionExerciseIds: ['trap_bar_deadlift'], substitutionExerciseIds: ['goblet_squat', 'box_squat', 'leg_press'] },
  ...
  barbell_bench_press: { regressionExerciseIds: ['dumbbell_bench_press', 'floor_press'], progressionExerciseIds: [], substitutionExerciseIds: ['dumbbell_bench_press', 'floor_press', 'push_up'] },
  ...
  barbell_row: { regressionExerciseIds: ['one_arm_dumbbell_row', 'band_row'], progressionExerciseIds: [], substitutionExerciseIds: ['one_arm_dumbbell_row', 'seated_cable_row', 'band_row'] },
  ...
  assisted_pull_up: { regressionExerciseIds: ['lat_pulldown', 'band_row'], progressionExerciseIds: ['pull_up'], substitutionExerciseIds: ['lat_pulldown', 'inverted_row'] },
  pull_up: { regressionExerciseIds: ['assisted_pull_up', 'lat_pulldown', 'band_row'], progressionExerciseIds: [], substitutionExerciseIds: ['assisted_pull_up', 'lat_pulldown', 'band_row'] },
  ...
  sit_up: { regressionExerciseIds: ['dead_bug'], progressionExerciseIds: ['front_plank'], substitutionExerciseIds: ['dead_bug', 'pallof_press'] },
  ...
  running_interval: { regressionExerciseIds: ['incline_walk'], progressionExerciseIds: [], substitutionExerciseIds: ['stationary_bike_zone2', 'assault_bike_zone2', 'rower_zone2'] },
  burpee: { regressionExerciseIds: ['bodyweight_squat', 'incline_push_up'], progressionExerciseIds: [], substitutionExerciseIds: ['sled_push', 'stationary_bike_zone2', 'incline_push_up'] },
...
function categoryFor(exercise: Exercise): NonNullable<Exercise['category']> {
  ...
  if (exercise.defaultPrescriptionTemplateId === 'conditioning_interval' || exercise.defaultPrescriptionTemplateId === 'hiit_interval') return 'conditioning';
  if (exercise.defaultPrescriptionTemplateId === 'zone2_steady') return 'cardio';
  if (exercise.defaultPrescriptionTemplateId === 'flexibility_hold') return 'flexibility';
...
function subPatternIdsFor(exercise: Exercise): string[] {
  ...
  for (const patternId of exercise.movementPatternIds) {
    ...
    if (patternId === 'trunk_flexion') ids.push('controlled_trunk_flexion');
...
function jointsFor(exercise: Exercise): string[] {
  ...
  if (includesAny(exercise.movementPatternIds, ['hinge', 'anti_extension', 'anti_rotation', 'trunk_flexion', 'rotation', 'crawl', 'breathing', 'thoracic_mobility'])) joints.push('spine');
...
function planeFor(exercise: Exercise): NonNullable<Exercise['planeOfMotion']> {
  ...
  if (exercise.movementPatternIds.includes('trunk_flexion')) planes.push('sagittal');
...
function setupTypeFor(exercise: Exercise): NonNullable<Exercise['setupType']> {
  ...
  if (exercise.equipmentIds.includes('mat') || includesAny(exercise.movementPatternIds, ['anti_extension', 'trunk_flexion', 'breathing'])) return 'floor';
...
function technicalComplexityFor(exercise: Exercise): NonNullable<Exercise['technicalComplexity']> {
  ...
  if (includesAny(exercise.movementPatternIds, ['hinge', 'lunge', 'carry', 'trunk_flexion', 'rotation', 'crawl']) || exercise.id === 'rower_zone2') return 'moderate';
  return 'low';
}
...
function spineLoadingFor(exercise: Exercise): NonNullable<Exercise['spineLoading']> {
  ...
  if (exercise.movementPatternIds.includes('trunk_flexion')) return 'moderate';
  return 'none';
}
...
function prescriptionRangesFor(exercise: Exercise): NonNullable<Exercise['defaultPrescriptionRanges']> {
  ...
  if (exercise.defaultPrescriptionTemplateId === 'hiit_interval') return { rounds: { min: 4, max: 8 }, workSeconds: { min: 20, max: 40 }, restIntervalSeconds: { min: 40, max: 90 }, rpe: { min: 7, max: 9 } };
  ...
  if (exercise.defaultPrescriptionTemplateId === 'flexibility_hold') return { sets: { min: 1, max: 3 }, holdSeconds: { min: 20, max: 45 }, rpe: { min: 1, max: 3 }, targetJoints: jointsFor(exercise), targetTissues: exercise.primaryMuscleIds, rangeOfMotionIntent: 'Relax into a pain-free stretch while preserving pelvis and rib position.' };
...
function trackingMetricsFor(exercise: Exercise): string[] {
  ...
  if (exercise.defaultPrescriptionTemplateId === 'conditioning_interval' || exercise.defaultPrescriptionTemplateId === 'hiit_interval') additions.push('rounds_completed', 'work_seconds', 'actual_rpe');
...
export const exercises: Exercise[] = [
  ex({ id: 'barbell_back_squat', name: 'Barbell Back Squat', summary: 'Rack-based loaded squat for strength development.', coachingSummary: 'Brace hard, keep the bar path over mid-foot, and use a depth that stays pain-free.', movementPatternIds: ['squat'], primaryMuscleIds: ['quads', 'glutes'], secondaryMuscleIds: ['adductors', 'spinal_erectors', 'core'], equipmentIds: ['barbell', 'squat_rack'], workoutTypeIds: ['strength', 'full_body_strength', 'lower_strength'], goalIds: ['full_gym_strength', 'lower_body_strength'], minExperience: 'intermediate', intensity: 'hard', impact: 'none', contraindicationFlags: ['knee_caution', 'back_caution'], trackingMetricIds: ['sets_completed', 'reps_completed', 'load_used', 'actual_rpe', 'rest_seconds'], defaultPrescriptionTemplateId: 'strength_heavy' }),
  ...
  ex({ id: 'barbell_bench_press', name: 'Barbell Bench Press', summary: 'Rack-based horizontal press for upper-body strength.', coachingSummary: 'Set the shoulder blades, keep wrists stacked, and press without shoulder pinch.', movementPatternIds: ['horizontal_push'], primaryMuscleIds: ['chest', 'triceps'], secondaryMuscleIds: ['shoulders'], equipmentIds: ['barbell', 'bench'], workoutTypeIds: ['strength', 'upper_strength'], goalIds: ['full_gym_strength', 'upper_body_strength'], minExperience: 'intermediate', intensity: 'hard', impact: 'none', contraindicationFlags: ['shoulder_caution', 'wrist_caution'], trackingMetricIds: ['sets_completed', 'reps_completed', 'load_used', 'actual_rpe', 'rest_seconds'], defaultPrescriptionTemplateId: 'strength_heavy' }),
  ...
  ex({ id: 'barbell_row', name: 'Barbell Row', summary: 'Unsupported barbell horizontal pull for upper-back strength.', coachingSummary: 'Hinge only as far as the trunk stays braced and row without yanking.', movementPatternIds: ['horizontal_pull', 'hinge'], primaryMuscleIds: ['upper_back', 'lats'], secondaryMuscleIds: ['biceps', 'spinal_erectors', 'forearms'], equipmentIds: ['barbell'], workoutTypeIds: ['strength', 'upper_strength'], goalIds: ['full_gym_strength', 'upper_body_strength'], minExperience: 'intermediate', intensity: 'hard', impact: 'none', contraindicationFlags: ['back_caution'], trackingMetricIds: ['sets_completed', 'reps_completed', 'load_used', 'actual_rpe'], defaultPrescriptionTemplateId: 'strength_heavy' }),
  ...
  ex({ id: 'pull_up', name: 'Pull-Up', summary: 'Bodyweight vertical pull for upper-body strength.', coachingSummary: 'Pull with shoulder blades set and stop before reps turn into neck tension.', movementPatternIds: ['vertical_pull'], primaryMuscleIds: ['lats', 'upper_back'], secondaryMuscleIds: ['biceps', 'forearms', 'core'], equipmentIds: ['pull_up_bar'], workoutTypeIds: ['strength', 'upper_strength', 'bodyweight_strength'], goalIds: ['upper_body_strength', 'full_gym_strength'], minExperience: 'intermediate', intensity: 'hard', impact: 'none', contraindicationFlags: ['shoulder_caution'], trackingMetricIds: ['sets_completed', 'reps_completed', 'actual_rpe'], defaultPrescriptionTemplateId: 'strength_heavy' }),
  ...
  ex({ id: 'sit_up', name: 'Sit-Up', summary: 'Trunk flexion exercise for anterior core endurance.', coachingSummary: 'Curl up under control and stop if the low back or hip flexors take over.', movementPatternIds: ['trunk_flexion'], primaryMuscleIds: ['rectus_abs'], secondaryMuscleIds: ['hip_flexors', 'obliques'], equipmentIds: ['bodyweight', 'mat'], workoutTypeIds: ['core_durability', 'bodyweight_strength'], goalIds: ['core_durability', 'no_equipment'], minExperience: 'beginner', intensity: 'moderate', impact: 'none', contraindicationFlags: ['back_caution'], trackingMetricIds: ['sets_completed', 'reps_completed', 'actual_rpe'], defaultPrescriptionTemplateId: 'core_control' }),
  ...
  ex({ id: 'running_interval', name: 'Running Interval', summary: 'Run-based conditioning interval.', coachingSummary: 'Keep stride relaxed and stop if impact or breathing becomes unrepeatable.', movementPatternIds: ['locomotion'], primaryMuscleIds: ['aerobic_system', 'calves'], secondaryMuscleIds: ['quads', 'glutes', 'hamstrings'], equipmentIds: ['bodyweight', 'track_or_road'], workoutTypeIds: ['conditioning'], goalIds: ['boxing_support', 'low_impact_conditioning'], minExperience: 'intermediate', intensity: 'hard', impact: 'high', contraindicationFlags: ['no_running', 'knee_caution'], trackingMetricIds: ['duration_minutes', 'pace', 'actual_rpe'], defaultPrescriptionTemplateId: 'hiit_interval' }),
  ex({ id: 'burpee', name: 'Burpee', summary: 'Bodyweight squat, floor transition, push-up, and jump conditioning movement.', coachingSummary: 'Use only when jumping, wrists, shoulders, and floor transitions are all tolerated.', movementPatternIds: ['squat', 'horizontal_push', 'jump_land'], primaryMuscleIds: ['full_body', 'aerobic_system'], secondaryMuscleIds: ['chest', 'triceps', 'quads', 'core'], equipmentIds: ['bodyweight', 'open_space'], workoutTypeIds: ['conditioning', 'bodyweight_strength'], goalIds: ['no_equipment', 'boxing_support'], minExperience: 'intermediate', intensity: 'hard', impact: 'high', contraindicationFlags: ['no_jumping', 'wrist_caution', 'shoulder_caution', 'knee_caution'], trackingMetricIds: ['rounds_completed', 'reps_completed', 'actual_rpe', 'movement_quality'], defaultPrescriptionTemplateId: 'hiit_interval' }),
...
export const prescriptionTemplates: PrescriptionTemplate[] = [
  {
    ...
    kind: 'resistance',
    appliesToWorkoutTypeIds: ['strength', 'bodyweight_strength', 'full_body_strength'],
    defaultSets: 3,
    defaultReps: '6-10',
    defaultRpe: 6,
    restSeconds: 90,
    ...
    progressionRuleIds: ['progression_beginner_linear_load', 'progression_autoregulated_rpe_rir'],
    payload: {
      kind: 'resistance',
      sets: { min: 2, max: 4, target: 3 },
      reps: '6-10',
      repRange: { min: 6, max: 10 },
      loadGuidance: 'Use a load or variation that allows crisp reps with 3-4 reps in reserve.',
      intensityModel: 'rpe',
      RPE: { min: 5, max: 7, target: 6 },
      RIR: { min: 3, max: 4 },
      restSecondsRange: { min: 60, max: 120, target: 90 },
      tempo: 'controlled',
      effortGuidance: 'Stop the set before form changes or pain appears.',
      mainLiftVsAccessory: 'main_lift',
      weeklyVolumeTarget: { min: 6, max: 10, unit: 'sets_per_pattern' },
      progressionRuleIds: ['progression_beginner_linear_load', 'progression_autoregulated_rpe_rir'],
    },
  },
  {
    ...
    kind: 'resistance',
    appliesToWorkoutTypeIds: ['strength', 'lower_strength', 'upper_strength', 'full_body_strength'],
    defaultSets: 4,
    defaultReps: '3-6',
    defaultRpe: 7,
    restSeconds: 150,
    ...
    progressionRuleIds: ['progression_double_6_10', 'progression_power_quality_gate'],
    payload: {
      kind: 'resistance',
      sets: { min: 3, max: 5, target: 4 },
      reps: '3-6',
      ...
      RPE: { min: 6, max: 8, target: 7 },
      RIR: { min: 2, max: 3 },
      percent1RM: { min: 70, max: 85 },
      restSecondsRange: { min: 120, max: 180, target: 150 },
      tempo: 'controlled eccentric, forceful concentric',
      effortGuidance: 'End the set if speed drops sharply or bracing changes.',
      mainLiftVsAccessory: 'main_lift',
      weeklyVolumeTarget: { min: 4, max: 8, unit: 'hard_sets_per_lift' },
      progressionRuleIds: ['progression_double_6_10', 'progression_autoregulated_rpe_rir'],
    },
  },
  {
    ...
    progressionRuleIds: ['progression_double_8_12', 'progression_hypertrophy_volume'],
    payload: {
      kind: 'resistance',
      sets: { min: 2, max: 4, target: 3, unit: 'sets_per_exercise' },
      reps: '8-12',
      repRange: { min: 8, max: 12 },
      loadGuidance: 'Select a load that reaches the target muscle with 1-3 reps in reserve.',
      intensityModel: 'rir',
      RPE: { min: 7, max: 9, target: 8 },
      RIR: { min: 1, max: 3, target: 2 },
      restSecondsRange: { min: 60, max: 90, target: 75 },
      tempo: '2-0-2',
      ...
      weeklyVolumeTarget: { min: 8, max: 14, unit: 'sets_per_muscle_group' },
      progressionRuleIds: ['progression_double_8_12', 'progression_hypertrophy_volume'],
    },
  },
  {
    ...
    kind: 'resistance',
    appliesToWorkoutTypeIds: ['hypertrophy', 'upper_strength'],
    defaultSets: 2,
    defaultReps: '12-15',
    defaultRpe: 7,
    restSeconds: 45,
    ...
    payload: {
      kind: 'resistance',
      sets: { min: 2, max: 3, target: 2, unit: 'sets_per_exercise' },
      reps: '12-15',
      ...
      RPE: { min: 6, max: 8, target: 7 },
      RIR: { min: 1, max: 3, target: 2 },
      restSecondsRange: { min: 30, max: 60, target: 45 },
      ...
      weeklyVolumeTarget: { min: 4, max: 10, unit: 'sets_per_accessory_pattern' },
      progressionRuleIds: ['progression_double_8_12'],
    },
  },
  {
    id: 'power_quality',
    ...
    progressionRuleIds: ['progression_power_quality_gate', 'progression_skill'],
    payload: {
      kind: 'power',
      sets: { min: 3, max: 5, target: 4 },
      reps: { min: 2, max: 5, target: 3 },
      explosiveIntent: 'Move explosively while staying technically clean.',
      fullRecoverySeconds: { min: 90, max: 180, target: 120 },
      technicalQuality: 'Stop if speed, landing, posture, or coordination drops.',
      lowFatigue: true,
      movementSpeed: 'fast and crisp; no grinding',
      eligibilityRestrictions: ['No red readiness state', 'No acute pain', 'No high fatigue trend'],
    },
  },
  {
    ...
    progressionRuleIds: ['progression_hiit_interval', 'progression_circuit_density'],
    payload: {
      kind: 'conditioning',
      workIntervalSeconds: { min: 20, max: 45, target: 40 },
      restIntervalSeconds: { min: 30, max: 60, target: 40 },
      rounds: { min: 4, max: 8, target: 6 },
      targetIntensity: { RPE: { min: 6, max: 8, target: 7 }, talkTest: 'Short phrases during work, full sentences during recovery.' },
      impactLevel: 'low',
  ...
  },
  {
    ...
    progressionRuleIds: ['progression_zone2_duration', 'progression_aerobic_frequency'],
    payload: {
      kind: 'cardio',
      durationMinutes: { min: 20, max: 45, target: 24 },
      modality: 'mixed_low_impact',
      heartRateZone: { min: 2, max: 2, unit: 'zone' },
      RPE: { min: 3, max: 5, target: 4 },
      talkTest: 'Can speak in full sentences without gasping.',
      pace: { target: 'conversational' },
  ...
  },
  {
    ...
    kind: 'recovery',
    appliesToWorkoutTypeIds: ['recovery'],
    ...
    payload: {
      kind: 'recovery',
      intensityCap: { max: 3, unit: 'RPE' },
      durationMinutes: { min: 8, max: 25, target: 12 },
  ...
  },
  {
    ...
    payload: {
      kind: 'mobility',
      targetJoints: ['hips', 'thoracic_spine', 'ankles', 'shoulders'],
      rangeOfMotionIntent: 'Expand usable range with control, not passive forcing.',
      reps: { target: '4-8 controlled reps per side' },
      holdTimeSeconds: { min: 10, max: 30 },
  ...
  },
  {
    ...
    kind: 'recovery',
    appliesToWorkoutTypeIds: ['recovery', 'mobility'],
    ...
    payload: {
      kind: 'recovery',
      intensityCap: { max: 2, unit: 'RPE' },
      durationMinutes: { min: 2, max: 6, target: 4 },
  ...
  },
  {
    ...
    kind: 'mobility',
    appliesToWorkoutTypeIds: ['mobility', 'recovery', 'boxing_support'],
    defaultSets: 2,
    defaultReps: '12-15',
    defaultRpe: 3,
    restSeconds: 30,
  ...
  },
  {
    ...
    label: 'Carry Control',
    kind: 'resistance',
    appliesToWorkoutTypeIds: ['strength', 'core_durability'],
    defaultSets: 3,
    ...
    progressionRuleIds: ['progression_core_endurance', 'progression_unilateral_stability'],
    payload: {
      kind: 'resistance',
      sets: { min: 2, max: 4, target: 3 },
      repRange: { target: '20-45 seconds per carry' },
      loadGuidance: 'Use a load that challenges posture without forcing a lean.',
      intensityModel: 'rpe',
      RPE: { min: 5, max: 7, target: 6 },
      restSecondsRange: { min: 45, max: 90, target: 60 },
  ...
  },
  {
    ...
    label: 'Core Control',
    kind: 'balance',
    appliesToWorkoutTypeIds: ['core_durability', 'strength', 'recovery'],
    defaultSets: 2,
    ...
    progressionRuleIds: ['progression_core_endurance', 'progression_balance_complexity'],
    payload: {
      kind: 'balance',
      ...
      mode: 'static',
      durationSeconds: { min: 20, max: 40, target: 30 },
      complexityProgression: ['increase hold quality', 'narrow base of support', 'add limb movement', 'add light load'],
      fallRiskRules: ['Use external support when balance is uncertain.', 'Do not progress if dizziness or instability appears.'],
    },
  },
  {
    id: 'flexibility_hold',
    ...
    payload: {
      kind: 'flexibility',
      targetTissues: ['hip_flexors', 'calves', 'lats', 'adductors'],
      targetJoints: ['hips', 'ankles', 'shoulders', 'thoracic_spine'],
      holdTimeSeconds: { min: 20, max: 45, target: 30 },
  ...
  },
  {
    ...
    kind: 'interval',
    appliesToWorkoutTypeIds: ['conditioning', 'boxing_support'],
    defaultSets: 6,
    defaultDurationSeconds: 30,
    defaultRpe: 8,
    restSeconds: 60,
    ...
    progressionRuleIds: ['progression_hiit_interval'],
    payload: {
      kind: 'interval',
      workIntervalSeconds: { min: 20, max: 40, target: 30 },
      restIntervalSeconds: { min: 40, max: 90, target: 60 },
      rounds: { min: 4, max: 8, target: 6 },
      targetIntensity: { RPE: { min: 7, max: 9, target: 8 }, talkTest: 'Broken phrases only during work; breathing should recover before the next round.' },
