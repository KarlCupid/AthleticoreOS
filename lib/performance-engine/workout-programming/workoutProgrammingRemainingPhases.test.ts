...
(() => {
  ...
  assert('phase 13 red readiness route uses return-to-training phase', redValidation.valid && redReadiness.weeks[0]?.phase === 'return_to_training' && redReadiness.weeks[0]?.sessions.every((session) => session.workout?.workoutTypeId === 'recovery'));
  assert('phase 13 yellow readiness after red preserves low-load frequency', redReadiness.weeks[1]?.sessions.some((session) => session.plannedIntensity === 'low' || session.plannedIntensity === 'recovery') === true && (redReadiness.weeks[1]?.weeklyVolumeSummary.generatedSessionCount ?? 0) > 0);
