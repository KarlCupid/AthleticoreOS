...
import { COLORS, FONT_FAMILY, SPACING, RADIUS, SHADOWS, GRADIENTS } from '../theme/theme';
...
export function ActiveCampBanner({ goalMode }: ActiveCampBannerProps) {
    ...
    // For now, if they have an active fight camp, we assume it's "Cut" or "Build". Let's assume it's a 4-week cut state.
    // In a real scenario, this would compute the distance to the target date.
    const weeksOut = 4;
    const progressPct = 0.35; // Example 35% through camp

    return (
        <AnimatedPressable onPress={() => navigation.navigate("PlanningHome")}>
            <View style={styles.container}>
                <LinearGradient
                    colors={[...GRADIENTS.caution]} // Deep amber gradient
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 1 }}
